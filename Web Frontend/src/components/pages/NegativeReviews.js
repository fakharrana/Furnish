...
import "./style/negativeReviews.css";
...
const NegativeReviews = (props) => {
  ...
  //Function for filtering negative reviews by date analyzed or product name
  ...
  //Function for retrieving negative reviews from server
  const getNegativeReviews = async () => {
    try {
      await authAxios.get("/negativereviews").then(
        function (response) {
  ...
  const deleteReview = async (reviewID) => {
    try {
      await authAxios.delete(`/negativereviews/deletereview/${reviewID}`).then(
        function (response) {
  ...
  //Function for deleting all the negative reviews
  const deleteAllReviews = async () => {
    try {
      await authAxios.delete("/negativereviews/deleteallreviews").then(
        function (response) {
          toast.success(response.data.Success, {
            position: "top-center",
            autoClose: 2000,
  ...
  return (
    <div className="negative-reviews">
      <Row className="top-row-negative-reviews">
        <Col>
          <h4 className="top-row-title-negative-reviews">
            <b>Negative Reviews ({data.length})</b>
          </h4>
        </Col>
        <Col>
          <input
            id="filter-by-product-name"
            className="search-bar-negative-reviews shadow "
            type="text"
        ...
        <Col>
          <Link
            ...
            className="btn btn-danger deleteAll-negative-reviews"
          >
            Delete All
          </Link>
        </Col>
      ...
      <Table hover className="shadow" size="sm">
        <thead className="thead-negative-reviews">
          <tr>
            <th>#</th>
            <th>
              Product Name
      ...
      {isLoading ? (
        ...
        <div className="review-not-found-negative-reviews">
          {" "}
